...
class AttendanceRecord(models.Model):
    """
    Модель для хранения записей посещаемости сотрудников.
    Связывает записи входов/выходов с графиками работы и определяет аномалии.
    """
    employee = models.ForeignKey(
        'Employee',
        on_delete=models.CASCADE,
    ...
    entry_exit = models.OneToOneField(
        'EntryExit',
        on_delete=models.CASCADE,
    ...
    is_late = models.BooleanField(
        ...
        help_text="Сотрудник опоздал на работу",
        db_index=True,
    )
    is_early_leave = models.BooleanField(
        default=False,
        verbose_name="Ранний уход",
        help_text="Сотрудник ушел раньше времени",
        db_index=True,
    )
    crosses_midnight = models.BooleanField(
        default=False,
        verbose_name="Смена через полночь",
        help_text="Смена пересекает полночь",
    )
    is_extra_shift = models.BooleanField(
        default=False,
        verbose_name="Дополнительная смена",
        help_text="Смена вне расписания",
    )
    ...
    anomalies = models.JSONField(
        ...
        help_text="Список обнаруженных аномалий",
    )
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=["employee", "actual_start_time"]),
            models.Index(fields=["is_late", "is_early_leave"]),
            models.Index(fields=["crosses_midnight"]),
        ]
    
    def __str__(self):
        return f"AttendanceRecord {self.id} - {self.employee.name} - {self.actual_start_time}"